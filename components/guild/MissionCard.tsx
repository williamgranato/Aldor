...
export default function MissionCard({ mission, onAccept, onLoopToggle, looping, active, disabled }:{ mission:any; onAccept:()=>void; onLoopToggle:()=>void; looping:boolean; active:boolean; disabled?:boolean; }){
  ...
  return (
    <motion.div whileHover={{scale: disabled?1:1.02}} className={"p-4 rounded-xl border shadow-md " + (disabled?'bg-slate-900/30 border-slate-800 opacity-70':'bg-slate-900/60 border-amber-900/40')}>
      <div className="flex justify-between items-center">
        <div>
          <div className="font-semibold">{mission.title || mission.name} <span className="ml-2 text-xs px-2 py-0.5 rounded bg-amber-800/40 border border-amber-700/40">{mission.rank}</span></div>
