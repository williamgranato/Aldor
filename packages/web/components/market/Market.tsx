...
export function Market(){
  ...
  const [query,setQuery]=useState(''); const [fType,setFType]=useState<typeof FILTERS.type[number]>('all'); const [fRarity,setFRarity]=useState<typeof FILTERS.rarity[number]>('all');
  const { state:marketState, seed, canAfford, buy } = useDailyMarket({ worldDateMs: state.world.dateMs, playerLevel: state.player.level, coins: state.player.coins, equipped: state.player.equipped, slotId, season: state.world?.season, weather: state.world?.weather, temperature: state.world?.temperature, providerActions: actions });
  const items = useMemo(()=>{ let arr=marketState?.items??[]; if(query) arr=arr.filter(i=>i.name.toLowerCase().includes(query.toLowerCase())); if(fType!=='all') arr=arr.filter(i=>i.type===fType); if(fRarity!=='all') arr=arr.filter(i=>i.rarity===fRarity); return arr; },[marketState,query,fType,fRarity]);
  function onBuy(id:string){ const res = buy(id); }
  return (<div className="p-4 md:p-6">
    <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
      <div><h1 className="text-2xl font-bold">Mercado</h1><div className="text-xs opacity-70">Rotação diária — seed {seed}</div></div>
      <div className="flex items-center gap-2"><div className="text-xs opacity-70">Reputação</div><div className="w-40 h-3 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{width:`${marketState?.rep ?? 0}%`}}/></div>{marketState&&(<span className="text-xs opacity-80 ml-2">{marketState.rep<34?'0% desc.':marketState.rep<67?'1% desc.':marketState.rep<100?'3% desc.':'5% desc.'}</span>)}</div>
    </div>
