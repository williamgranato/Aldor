...
export function MarketModal({ item, onClose, onBuy, canAfford, equippedCompare }:{ item:GeneratedMarketItem|null; onClose:()=>void; onBuy:(id:string)=>void; canAfford:(p:any)=>boolean; equippedCompare?:any }){
  ...
  if(!item) return null; const price=item.discountedPrice??item.price; const afford=canAfford(price);
  const chip:Record<string,string>={common:'bg-gray-600/40',uncommon:'bg-emerald-600/40',rare:'bg-sky-600/40',epic:'bg-violet-600/40',legendary:'bg-rose-600/40'};
      ...
      <div className="flex gap-6 flex-col md:flex-row">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded ${chip[item.rarity]||'bg-gray-600/40'}`}>{item.rarity}</span>
